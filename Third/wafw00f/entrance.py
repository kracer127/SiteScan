...
from Third.wafw00f.lib.asciiarts import *
from Third.wafw00f import __version__, __license__
from Third.wafw00f.manager import load_plugins
from Third.wafw00f.wafprio import wafdetectionsprio
from Third.wafw00f.lib.evillib import urlParser, waftoolsengine, def_headers
...
def main(allDict, target):
    ...
    if not target.endswith('/'):
        target = target + '/'
    attacker = WAFW00F(target, debuglevel=0, path='/',
                followredirect=True, extraheaders={},
                    proxies=None)
